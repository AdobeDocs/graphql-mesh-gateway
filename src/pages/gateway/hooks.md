...
- `composer` (string) - The local or remote file location of the function you want to execute.
    ...
    You must add any local scripts to the mesh's [`files` array](./source-handlers.md#reference-local-files-in-handlers). [Local vs remote functions](#local-vs-remote-functions) describes when to use a local or remote function.
...
Local composers require adding any local scripts to the mesh's [`files` array](./source-handlers.md#reference-local-files-in-handlers).
