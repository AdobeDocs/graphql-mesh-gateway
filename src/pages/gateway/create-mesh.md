...
The `aio api-mesh:source` commands provide several prebuilt mesh templates that you can use to create your mesh file, for example `mesh.json`. Each template contains a mesh file designed for a specific first or third-party source. Third-parties can submit their sources as a pull request to the [api-mesh-sources](https://github.com/adobe/api-mesh-sources) repo. Once approved, these sources will be available for selection in the CLI.

Currently this feature serves as a way for you to quickly copy an example mesh.

1. To choose a configuration template from a list, run the following command:
