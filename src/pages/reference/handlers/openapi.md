...
And transmit it to the Rest API as an `Authorization` header. GraphQL Mesh does not allow dynamic selection in the `mesh.json` file, but that's fine! We can use a bit of trickery.
